...
class RangeSlider(ActiveImageControl):
    ...
    def __init__(self, parent, bitmaps, is_vertical=False, is_inverted=False, *args, **kwargs):
        ...
        self._handle_lo_offset = (0, 0)  # x,y offset for positioning handle relative to the zero position
        self._handle_lo_max_pos = wx.Point(self._static_size[0], self._static_size[1])  # max pos in relation to zero pos
        self._handle_lo_default = wx.Point(0, 0)
        self._handle_lo_pos = wx.Point(self._handle_lo_offset)
        print(self._handle_lo_pos)

        self.handle_hi_bmp = bitmaps[1]
        self._handle_hi_size = self.handle_hi_bmp.Size
        self._handle_hi_centre = rect_centre(self._handle_hi_size)
        self._handle_hi_offset = (0, 0)  # x,y offset for positioning handle relative to the zero position
        self._handle_hi_max_pos = wx.Point(self._static_size[0], self._static_size[1])
        self._handle_hi_default = wx.Point(0, 0)
        self._handle_hi_pos = wx.Point(self._handle_hi_offset)
    ...
    def draw_to_context(self, dc):
        ...
        if self.inverted:
            position = self._handle_lo_max_pos[0] - self._handle_lo_pos[0] + self._static_pos[0], \
                         self._handle_lo_max_pos[1] - self._handle_lo_pos[1] + self._static_pos[1]
        else:
            position = self._handle_lo_pos[0] + self._static_pos[0], \
                         self._handle_lo_pos[1] + self._static_pos[1]
            print(self._handle_lo_pos)
        dc.DrawBitmap(self.handle_lo_bmp, position)
        ...
        if self.HasFocus():
            ...
            if keycode in [wx.WXK_RIGHT, wx.WXK_UP]:
                if (inverted and not vertical) or (vertical and not inverted):
                    self.move_handle(self._handle_lo_pos[index] - self._cursor_key_step)
                else:
                    self.move_handle(self._handle_lo_pos[index] + self._cursor_key_step)
    ...
    def on_mouse_wheel(self, event):
        ...
        if (inverted and not vertical) or (vertical and not inverted):
            self.move_handle(self._handle_lo_pos[index] - (self._scrollwheel_step * event.GetWheelRotation() // delta))
        else:
            self.move_handle(self._handle_lo_pos[index] + (self._scrollwheel_step * event.GetWheelRotation() // delta))
    ...
    def mouse_move(self, mouse_pos):
        if not self.HasFocus():
            self.SetFocus()
        index = self.vertical
        position = mouse_pos[index] - self._handle_lo_centre[index] - self._static_padding[index]
        if self.inverted:
            position = self._handle_lo_max_pos[index] - position
        self.move_handle(position)

    def move_handle(self, pos):
        if self.vertical:
            self.set_position((self._handle_lo_pos[0], pos))
        else:
            self.set_position((pos, self._handle_lo_pos[1]))
    ...
    def set_default_pos(self, pos=(0, 0)):
        ...
        if 0 <= pos[0] <= self._static_size[0]:  # checks that position is within image boundary
            ...
            self._handle_lo_default = self._handle_lo_pos
        else:
    ...
    def set_position(self, pos=(0, 0)):
        ...
        valid_pos = self._validate_limits(pos, self._handle_lo_max_pos)
        if valid_pos != self._handle_lo_pos:
            self._handle_lo_pos = valid_pos
            wx.PostEvent(self, rs_cmd_event(id=self.GetId(), value1=self.value1, value2=self.value2))
            self.Refresh(True)
    ...
    @property
    def value1(self):
        value1 = self._handle_lo_pos[self.vertical] / self._handle_lo_max_pos[self.vertical]
        return value1  # as percentage

    @value1.setter
    def value1(self, percent):
        if self.vertical:
            self.set_position((self._handle_lo_max_pos[0], percent * self._handle_lo_max_pos[1]))
        else:
            self.set_position((percent * self._handle_lo_max_pos[0], self._handle_lo_max_pos[1]))

    @property
    def value2(self):
        value2 = self._handle_hi_pos[self.vertical] / self._handle_hi_max_pos[self.vertical]
        return value2  # as percentage

    @value2.setter
    def value2(self, percent):
        if self.vertical:
            self.set_position((self._handle_hi_max_pos[0], percent * self._handle_hi_max_pos[1]))
        else:
            self.set_position((percent * self._handle_hi_max_pos[0], self._handle_hi_max_pos[1]))

    # Animation methods #
    def _animate(self, destination, animate=True):
        if animate:
            index = self.vertical
            curr_pos = self._handle_lo_pos[index]  # for horizontal movement, [1] for vertical...
            max_pos = self._handle_lo_max_pos[index]
